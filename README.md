...
3. `git clone`을 받아온 후 requirements.txt가 있는 폴더로 이동하여 다음 코드를 실행하여 패키지들을 설치한다.
```
pip install -r requirements.txt
```
4. react-haksul/에 들어가서 터미널에 아래 코드 실행
```
npm install
```
5. `subprocess` 모듈을 사용하여서 터미널에서 `python start_both.py`를 실행하여 Django와 React를 전부 실행한다.
6. 을 실행했을 때 아래와 같은 화면이 나오면 성공!
